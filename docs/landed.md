...
**A fourth pass on 2026-08-21 added four more** — `B.3`, `B.4`, `C.10`, and `N.8`,
which `B.4` closed without a row of its own by giving `workspace.folder()` a caller
on the existing create path. `B.5` landed short and stays in plan.md; `B.6` was
split into `B.6a`–`B.6c` and has not started.

**A third pass on 2026-08-20 added six more** — the two guards `T.6` / `T.7`, the
three repairs `C.9` / `C.10`'s sibling `C.11` and the completed `C.2` / `C.7`.
Everything the 2026-08-20 review turned up is now either archived here or
carried as `◐` in plan.md.

**A second pass on 2026-08-20 added four more** — `B.2`, `B.7`, `C.4` and `C.5` — from the batch
Clay reviewed that day. The other three rows in that batch (`C.2`, `C.7`, `N.1`) landed short and
are still in plan.md as `◐`.
...
| **B.2 ✓** | **LANDED 2026-08-20, driven.** `packages/base/definitions.yaml` ships eight definitions, `src/modules/block/` registers eight modules behind the `block` component, published in `modules/base.ts`, and `conformance.test.ts` runs the component contract over each. **One correction to the row**: it is *not* read by nothing — `packages/**/*.yaml` is an eager glob, so `pkg_base` joins the shipped catalog and is importable by name. It stays off the type strip (offerings lists only packages a project's `vocabulary` imports), so *the app is unchanged on screen* holds.  ~~**Ship the `base` package and the block-module registry.**~~ `workspace` / `project` / `folder` / `structure` / `view` / `resource` / `group` / `note` as definitions, each naming a module. Published in `modules/base.ts`, read by nothing yet | `packages/base/`, `src/modules/` | B.0 |
| **B.7 ✓** | **LANDED 2026-08-20, driven.** `ValueForm`'s `ref` is `link` through `graph/types.ts`, `page/Contents.tsx` and every `packages/` file; no `"ref"` token remains under `src/` or `packages/`.  ~~**`ref` value form → `link`.**~~ Small and independent; any time after `B.1` | `src/graph/`, `packages/` | B.1 |
| ~~**B.3**~~ **landed — 2026-08-20** | **Delete the kind derivation.** `page/kind.ts` is gone; which view modules a layer offers comes from the definition. `App` / `Files`, behavior inference, view registrations and their tests now use that model. Closes `P.6`'s one-way door and defects `1b`–`1h` | `page/kind.ts`, `page/App.tsx`, `page/Files.tsx`, `src/modules/view/index.ts`, `src/modules/view/{activity,sequence,state,table,matrix}/index.ts`, `src/actions/behavior.ts`, `tests/modules/view.test.ts`, `tests/modules/conformance.test.ts`, `tests/actions/behavior.test.ts` | B.2 |
| ~~**B.4**~~ **landed — 2026-08-20** | **`folder` becomes a definition**, `role_set` → `role_folder`, and the mixed-children reading comes out. `App.create()` routes workspace context to `workspace.folder()` and empty explorer space now has a folder create path; 43 workspace tests pass and the full suite is proven | `page/Files.tsx`, `src/modules/icons/`, `src/workspace/index.ts`, `src/page/App.tsx`, `tests/workspace.test.ts` | B.2 |
...
### Stream C — the explorer and the canvas answer back

*Landed and driven 2026-08-20, **out of turn** — stream `C` is wave 5 and wave 1 was still
running. `C.2` and `C.7` from the same batch landed short and stayed in plan.md; `C.2` broke
drawing a relationship, which is `C.11`.*

| | Does / what landed | Owns | Waits |
|---|---|---|---|
| **C.4 ✓** | **LANDED 2026-08-20, driven** — `boxOf` / `fitOf` in `diagram/viewport.ts`, `F` routed through the gesture map rather than hardcoded. Driven: with a card picked, `f` moved the camera from `scale(1.3)` to `scale(1.6)` and centred it.  ~~**`f` zooms to the selection and centres it.**~~ One key, the standard one, over whatever is picked — a card, a group, an edge, several. **The camera already has fit** (the viewport is part of every projection surface); what is missing is a binding and a target | `canvas/`, `modules/view/diagram/` | ⊘ |
| **C.5 ✓** | **LANDED 2026-08-20, driven** — four `rim` spans on the frame plus `nearestSide`, styled with the `P.14` lit-target look rather than a second one. Driven: hovering the left wall lit `rim-left` alone.  ~~**A frame edge highlights as a drop target, one side at a time.**~~ The frame has four **walls** and they are already the one border zone a gesture can land on, but nothing lights when a drag is over one — so dropping an interface onto a wall is aim-and-hope. Each side lights independently, matching the canvas's existing lit-target treatment (`P.14`) rather than inventing a second look | `modules/view/diagram/`, `src/styles.css` | ⊘ |
| **C.2 ✓** | **LANDED 2026-08-20; drawing remainder closed by C.11.** Both halves driven: one anchor per arriving line (2 on the stage, not 8), dragging it moves it between seats, a moved one draws solid, **and a relationship made now draws immediately**.  ~~**An anchor exists where a line meets a block, and is draggable between seats without promotion.** Two halves. **(a)** The always-four rule goes: an anchor is drawn where a relationship actually arrives, so a card with one line has one. **(b)** Dragging it moves it between the seats available on that border; **a moved anchor draws solid**, to say its position is yours rather than the engine's. **Promotion stays a separate act** — an interface is a real element with a name and a type, and moving a line's end is not that~~ | `modules/view/diagram/`, `src/actions/`, `src/geometry/` | ⊘ |
| **C.7 ✓** | **LANDED 2026-08-20; complete after C.9.** Driven: a cross-project reference reads its target name on a card and in a table and survives reload. A closed project reads **`closed`**, distinct from a gone target's **`missing`**. | `page/App.tsx`, `canvas/`, `page/Contents.tsx`, `modules/view/table/` | ⊘
| **C.9 ✓** | **LANDED 2026-08-20.** `shownName` / `stoodFor` moved to `src/modules/named.ts`; canvas is a re-export and table imports point downward. Lookup is passed `asTarget` plus `here`/`open`; closed projects read **`closed`**, gone targets **`missing`**. Scoped/full tests, typecheck, and browser driving passed. One-card merge is **C.10** | `src/modules/named.ts`, `src/canvas/named.ts`, `src/modules/view/table/`, `tests/modules/named.test.ts`, `tests/modules/view/table.test.ts` | ⊘
| **C.11 ✓** | **LANDED 2026-08-20.** Handles stated on the node from `laidOf`'s seat map, so the edge draws in the same pass. Driven: right-drag card to card, line visible before reload; one perch per arriving line; T.6's edge-in-DOM assertion green.  ~~**A new relationship draws the moment it is made.** `C.2` traded four static `auto-*` handles per card for one pair minted per arriving edge — and a handle that does not exist until the edge exists is a handle React Flow has never measured, so the edge is dropped and nothing draws. A reload fixes it, which is why the suite and a screenshot both look fine. **Do not go back to four per side** — that is what `C.2` deleted. The fix is to make the handle available to React Flow **before or with** the edge that names it: seat the anchors from `laidOf`'s seat map on the node's own data (they are already computed there), or let the edge fall back to a side handle for the frame it takes to mount. **Done when**: right-drag card to card and the line is there, with no reload, driven — and `C.2`'s one-anchor-per-arriving-line rule still holds~~ | `modules/view/diagram/compose.ts` | ⊘ |
| **C.10 ✓** | **One card, drawn once.** `C.7` left `src/canvas/NodeCard.tsx` (285 lines) as a near-copy of `modules/view/diagram/NodeCard.tsx` (348), differing only in `nameOf` → `shownName`, and `Canvas.tsx` overrides `NODES.card` with it — so **the module's own card no longer draws anything in the app while the suite still exercises it**. Two copies of one renderer, one live and one tested, is how they drift. **Clay, 2026-08-20: keep one, and move the tests to it.** The natural shape is the module's card taking the resolver rather than the name, which `C.9` is already moving. **Landed 2026-08-20:** the duplicate was deleted; the module card is the sole renderer, receives `shownName` through `compose` / `useOpen`, and the diagram tests cover it across projects | `src/canvas/NodeCard.tsx`, `src/canvas/Canvas.tsx`, `src/modules/view/diagram/NodeCard.tsx`, `src/modules/view/diagram/pieces.tsx`, `src/modules/view/diagram/compose.ts`, `tests/modules/view/diagram.test.ts` | C.9 |

### Stream N — back to the drawing board

*`N.1`'s redraw gap is still `◐` in plan.md and is **not** archived here.*

| | Does / what landed | Owns | Waits |
|---|---|---|---|
| ~~**N.8**~~ **closed 2026-08-21 by `B.4`** | ~~**A workspace folder can be made from the app.**~~ `workspace.folder()` has a caller: creating in the workspace context makes a folder through the **existing** create path (`page/App.tsx`'s `create`) — which is exactly what this row asked for, *do not invent a second create surface*. Nothing left to build. **`N.1`'s redraw gap is untouched by this** and is still `◐` | — | — |

### Wave 0 — the guards

*Written 2026-08-20 after a batch landed green, typechecked, and broke making a
relationship. **Both were red when written**, which is the point of them; `C.9`
turned `T.7` green and `C.11` turned `T.6` green. They own `tests/` alone, so
neither ever contends with an implementation row.*

| | Does / what landed | Owns | Waits |
|---|---|---|---|
| **T.6 ✓** | **LANDED 2026-08-20 (not driven).** `tests/canvas/layer.test.ts` mounts a layer. **`C.11` turned the remaining assertion green**: after a link the relationship is in the DOM. One perch per arriving relationship and none on a spare card already hold; a note still has its handles after the cards are related.  ~~**The canvas gets enough cover to see a broken canvas.**~~ `C.2` broke *making a relationship* — the line does not draw until a reload — and **763 green tests and `tsc` saw none of it**, because nothing in `tests/` mounts a layer. That is not a coverage gap in a file, it is a whole class of defect the suite cannot express, and it is the class every driven failure so far has belonged to (`P.15`, `V.14`, `ST.1`, `ST.3`, now `C.2` and `N.1`). **Deliberately small**: mount a layer through `T.5`'s harness and assert **properties, never values** — an edge is in the DOM after a link mutation; a card carries an anchor for each relationship arriving at it and none for those that do not; a note keeps its handles. **Not a page suite** — that is `T.3`, which waits on `B.9`. **This waits on nothing**: the diagram's node/edge composition is the oldest settled surface in the app, and `B` does not touch it. **Done when**: a suite that fails on today's tree, and passes once `C.11` lands | `tests/modules/view/diagram.test.ts`, `tests/canvas/` | ⊘ |
| **T.7 ✓** | **LANDED 2026-08-20 (not driven).** One test walks every import under `src/` against a single allowlist in `tests/structure.test.ts`; README.md points at that list. Now passes after C.9, naming `canvas/named.ts: canvas → workspace` and `modules/view/table/{columns,rows}.ts: modules → canvas`; legal imports do not trip it. C.9 inverted those arrows.  ~~**The dependency map is enforced, not described.**~~ README.md's table is the one structural rule CLAUDE.md calls *the easy accident*, and it is prose — so `C.7` shipped `modules/` importing `canvas/` and `canvas/` importing `workspace/`, and `tsc`, the suite and review-by-reading all passed it. **One test, walking the imports under `src/` against the table**, turns the rule into something that fails loudly the first time somebody points an arrow the wrong way. **Keep the table in one place**: the test reads a list it owns and README.md points at it, or the reverse — never two copies to drift. **Done when**: the test fails on today's tree (`C.9` is what makes it pass), names the offending file and the arrow it drew, and no legal import trips it | `tests/structure.test.ts`, `README.md` | ⊘ |
