...
**`Y.1`, `Y.2`, `Y.3` and `Y.4 ◐` have landed too** — one options column on the page's right, groups
declared by the open view, icons over words, and the view toggle and export moved out of the
explorer. **Startable now**: `Y.7` (drop `color`, add the dials), `Y.6`'s `svg.ts` remainder, and
`Y.4`'s `types` group. `Y.9` waits on `Y.7`.
