...
| **B.14** | **An interface is declared, not derived.** Retire the `side` derivation; `side` becomes only where it sits. Reaches `promotion`, which was already the explicit act | `src/graph/`, `modules/view/diagram/` | B.6c |
...
| **B.21** | **A view holds views, and a reference carries `depth`.** A matrix's two axes are child views; `self` / `children` / `all` says how far each reference reaches, which is what makes dragging a whole project in give you its blocks as rows and keep them live. **`ST.4`'s payoff, and it needs `B.6` under it** | `src/modules/view/`, `src/graph/` | B.6c, B.15 |
