...
The queue. One row is one chunk of work — small enough to land in a sitting. A row says **what it
does** and **what it waits on**; **which files it owns is decided when it is drafted**, not here, so
the list can never go stale against the tree.
...
| **ST.12** | **The plan stops being prose that agents parse.** **Drafted 2026-08-21 → [translator.md](translator.md)**, with the cost measured and a seven-row plan. Stories, rows, `owns`, `waits` and *done when* are a **schema**, not a document — and today they are a 1,100-line markdown file that four agents read by pattern-matching. The costs are already paid and recurring: an `owns` that omits a file its own *done when* names stops the queue dead (`C.10`, `B.3`, 2026-08-20); a note written for Clay is read by `order-queue` as a gate; a row cannot be split without a human transcribing three new rows by hand; and nothing anywhere checks a row against itself. **Probably its own project**, not a row here — a small tool over a structured plan, with this repo as its first consumer | none yet | **future story, Clay 2026-08-21.** Not started, not scoped. **Nothing in the current queue waits on it** — it is a response to the maintenance cost of `B`/`C`/`N`, not a prerequisite for them |
...
| **ST.8** | **Interaction is consistent, and the app always answers back.** Every part you can adjust by hand — a card, a frame edge, a block anchor, an interface — targets the same way, lights the same way, and moves smoothly. Nothing is aim-and-hope, and no two surfaces teach different gestures for the same act | `C.1` → `C.11` | **PARKED, Clay 2026-08-21 — revisit when the model has settled.** Driven and it is nowhere near: **a card dragged away leaves its relation lines behind**; **the tree does not redraw until focus changes**; **an anchor is 12×12 and near-impossible to hit, harder to drag**; and a relationship meeting an interface did not draw at all (fixed 2026-08-21, below). Each is its own row and the goal is *consistency*, which cannot be judged while `B` is still moving what things are. **`C.9`–`C.11` and `C.2`/`C.4`/`C.5`/`C.7` landed and stay landed**; `C.1`, `C.3`, `C.6`, `C.8 ◆` are parked with the story |
...
| **B.3** | **landed — 2026-08-20.** Deleted `page/kind.ts`. **Be precise about what this did**: `offered()` returns **all six** modules unconditionally, which closes `P.6`'s one-way door — the row's actual clause. *Which* modules a layer offers coming **from the definition** is **not** built; it is `B.15`'s, and `App.tsx`'s comment says so. `ViewKind` / `kindOf` / `createsFor` and view-module registration metadata are gone; a fresh project can switch to **activity**. Proven in the view, conformance and behavior tests and by the browser drive |
| **B.4** | **landed — 2026-08-20.** `workspace.folder()` emits a local `def_folder` via `set_def` + `add_element`; `role_set` is now `role_folder`, and `role_of` recognises the folder definition rather than mixed children. Proven: right-clicking empty explorer space creates a folder with the folder mark, while a container of unlike blocks does not |
| **B.5 ◐** | **landed — 2026-08-20.** `isReference`, `isContained` and `isPart` now derive the two link roles from the stored `of` link and root references; property tests cover parts, references and deletion-safe filed roots. **Remainder landed 2026-08-21**: one shared `cascades()` predicate now gates both the fold's `delete_element` sweep and `drop.run`'s own payload, so a contained root un-files instead of being deleted; both property tests rewritten against the folded graph. **Still short:** `isPart` has no caller in `src/` and provably cannot get one here — `isContained` implies `isReference` implies `!isPart` (defect 31) |
| **B.6a/b/c** | **split 2026-08-21** (Clay), and **not by base definition** — by *reader, then token*, which is what `B.5` proved works: it moved `isReference` off `form: "proxy"` while leaving the token on disk. Splitting note from group would have been two prove cycles for one behaviour; removing `form` first would have left the app undrivable in the middle, with notes and groups carrying neither a form nor a definition. **6a** ships the definitions, **6b** switches the 22 readers, **6c** removes the field |
...
**One migration, at the end.** `B.6c` changes the element record, `B.8` changes where the log lives,
...
| | Does | Waits |
|---|---|---|
| **B.5 ◐** | **LANDED SHORT 2026-08-20.** `fold.ts` derives `isReference` from the stored `of` link (not `form: "proxy"`, which stays the on-disk spelling — the right precedent, and `B.6` should follow it), plus `isContained` and `isPart`. **Three gaps.** (a) **Nothing in `src/` calls `isContained` or `isPart`** — every call site is in `tests/`, so containment is not derived anywhere in the running app; the predicates exist and are tested. Same shape as the `workspace.folder()` hole `B.4` has just closed. (b) **The property test does not test the property.** The row requires *deleting a container never deletes a contained root*; `tests/graph/fold.test.ts`'s *derives containment from a filed graph root* checks `isContained` against an **un-folded local object** (a pure-function check, not a graph one), asserts `ROOT` survives (it is undeletable by construction, so this always passes), and asserts the contained reference **is** deleted with its container — the opposite of the clause, passing. (c) `src/actions/elements.ts`'s mutation payload shape was left unchanged. **(b) and (c) closed 2026-08-21**, and (a) half — `isContained` gates the live cascade through `cascades()`, shared by `fold.ts` and `drop.run` so the two cannot drift; `isPart` still has none, and two attempts to give it one were caught in review as provably dead code | B.2 |
| **B.6a** | **`note` and `group` become base definitions, and the engine reads them.** `form` stays on the record and stays authoritative on disk. **Drivable at the end**: a note and a group still draw, now typed by a definition. **Landed 2026-08-21** — `groups.ts` sets `type` to `defIdFor("group"/"note")` and mints a matching local `set_def` (`form: "block"`, mirroring the shipped entry) when the project lacks one; the definitions and stub modules already shipped in `B.2`. Driven: a note and a group draw unchanged and the types list gained nothing. **Done when**: both ship in `packages/base/`, an element of each resolves through `resolved()`, and nothing on screen changed | B.5 |
| **B.6b** | **The readers switch.** The **22** `form === "note"` / `form === "group"` branches under `src/` read the definition instead; `form` becomes vestigial but is still written. **Drivable at the end**: same behaviour, one source of truth. **Done when**: no branch under `src/` reads `form` for note or group, and a note, a group and a tie all still draw | B.6a |
| **B.6c** | **`form` leaves the element record.** **This is the row that breaks standing rule 2 on purpose** — a schema token is `B.17`'s alone, and the plan already accepts that this stream is unshippable between here and `B.17`. **Nothing may ship between this row and `B.17`.** **Done when**: `ElemForm` is gone from `Element`, the door heals a log that still carries one, and `B.17` lands with it or immediately after | B.6b, and **`B.17` before shipping** |
...
| **B.9** | **Display state moves to workspace metadata** — explorer fold, canvas toggles, and which view each layer was last shown in. Reopening a workspace finds every project as it was left; an exported project carries no opinion about how it is drawn. **Reverses `U.8`'s `localStorage` on purpose** | B.8 |
| **B.11** | **One arrangement, six values, absorbing `axis` and `flow`.** `free` / `grid` / `right` / `left` / `down` / `up`; the four directional ones carry the reading direction. **Model data, in the log** — inference reads it, and an inference is permanent. **`radial` is dropped** (narrow, wrong-looking outside a hub and its attendants) and **`relax` is retired outright** — the action, the `relax_layer` mutation op and its `fill.ts` entry all go, since *hand it back to automatic* has nothing left to mean once arrangement is a setting. **Named loss**: nothing clears hand placement any more. **Watch**: `implied order` is read along it, so `A.9`'s sequence and `A.7b`'s activity must keep their fallback, and **tier 3 must stop firing under `free` and `grid`** | B.0 |
| **B.12** | **Arrangement becomes a setting**, not a one-time action. **Reverses *an arrangement is never a mode*** — deliberately, because `relax` cannot be anything else. **`at` is never discarded**: a computed arrangement replaces where things draw, and returning to `free` returns the layout (*retained placement*, already the rule) | B.11 |
| **B.13** | **`derived` as a relationship flag.** Not in the log, recomputed on fold, not deletable. The relation module set stays `line` / `directed`. First consumer is the workspace's project-to-project dependencies, which are drawn today and stored nowhere | B.5 |
| **B.14** | **An interface is declared, not derived.** Retire the `side` derivation; `side` becomes only where it sits. Reaches `promotion`, which was already the explicit act | B.6c |
| **B.15** | **View definitions become their own records** — a **view subtype**: one required view module plus its options, reusable rather than copied into every definition. A block definition carries **`views`, an ordered list of them, first entry the default** — one field, because which view opens is a presentation detail. The base package ships a trivial view definition per module so every entry is the same kind of thing | B.2 |
| **B.16** | **The module schema is enforced at import, and an unknown module reports a fault.** A definition must match the registered option surface of the module it names; a module the build does not know falls back to the base block **and says so**. Never silent — a file from a newer build must not open looking subtly wrong with nothing to explain it | B.2, B.15 |
| **B.20** | **A `resource` block carries a workspace-relative path or link**, and the explorer and canvas draw one. A file, a script, a data file, an image, a note. **Embedded content is out of scope** — a path only, so the log never carries bytes | B.2 |
| **B.21** | **A view holds views, and a reference carries `depth`.** A matrix's two axes are child views; `self` / `children` / `all` says how far each reference reaches, which is what makes dragging a whole project in give you its blocks as rows and keep them live. **`ST.4`'s payoff, and it needs `B.6` under it** | B.6c, B.15 |
| **B.22** | **Pin a layer view as a view block.** A pin at the canvas's top right; the block holds references to what is on stage, names the view definition in force, and is filed like anything else. **The other half of `ST.4`'s payoff** — `P.4` made a set from the explorer, this makes one from what you are looking at | B.21 |
| **B.23** | **Wire `create` and `infer` to `word` and `creates`.** Parked since **A.7c** — `ViewModule` carries both and nothing reads them, so a right-click in an activity makes a plain block rather than an action. Not a new capability; a wire that was never run | B.15 |
| **B.24** | **Filing a block promotes it.** The rule `B.5` derives *contained* from — dragging a loose block into a folder or onto the workspace retypes it to `project`, dragging it into a project files it as a part. **`P.14` already dissolved *beside*: every drop is *into* something**, so this is that rule finishing rather than a new gesture | B.5 |
| **B.25** | **The explorer hides empty blocks.** Basic blocks, interfaces and references are usually empty, and hiding them is what makes a large tree readable. **A project or package root is always shown, even when empty.** Display state, so it lives in workspace metadata | B.9 |
| **B.26** | **The fold merges checkpointed definitions with folded ones.** A package arrives as a checkpoint, not as steps, so its definitions are not in the workspace log — the fold has to read both. Checkpoints already carry a graph, so this is probably a merge and a lookup order, but *probably* is how the last two schema surprises started | B.8 |
| **B.27** | **Every minted project goes through `workspace.begin`.** tasks.md defect **2**: a project minted by `infer` (and by anything else reaching `writeInto` + `onAdmit` directly) gets a root labelled `""`, so it draws blank — and it **skips `mayName`**, the duplicate check `workspace.begin` exists to enforce. `P.3` made three paths into one *admit* door; this makes them one *naming* door too. **Done when**: no path can make a project without a name, a duplicate name is refused wherever it comes from, and `infer`'s result is named | B.8 |
| **B.18** | **View composition, on one metric: proximity.** How far apart two referenced blocks sit in the tree — same parent, same branch, same project, different project — **groups** by nearest common ancestor, **orders** by tree path, and **spaces** by distance where the view has room. Block takes all three; table and matrix take grouping and order and drop the spacing; matrix applies it per axis, each axis being a child view. A proximity group is a **derived group**, stored nowhere. **Nothing answers this today**, and it is what a view block full of distant references needs to be readable. Not `infer` — it makes no blocks and is recomputed every draw. **The default must be overridable**: a cross-cut view wants grouping by type, not by project | B.15 |
| **B.17** | **Migrate old files, once, covering every change in this stream.** Schema `1.2` writes `form: "proxy"` / `form: "note"` and a log per project. The door already heals a legacy domain stem, so heal there rather than bumping by fiat. **Nothing in this stream ships without it** — a build that cannot read yesterday's file is the one unrecoverable outcome in this stream | every other B row |
...
`ST.4`'s payoff rather than its plumbing, so they sit after `B.6c` — but they are the two rows that
...
| | Rows | Story | Why here |
|---|---|---|---|
| ~~**0**~~ | `T.6` `T.7` | — | **done 2026-08-20 — [landed.md](landed.md)**. The guards. Both were red when written; `C.9` and `C.11` turned them green |
| **1** | `B.5` ◐, `B.6a` → `B.9` | `ST.4` | the model core (`B.2`, `B.3`, `B.4` landed — [landed.md](landed.md)). Everything later is written against it, and **`B.8` carries live defect 1** (a write to the wrong project's log), so it is not deferrable |
...
| **5** | `C.1`, `C.3`, `C.6`, `C.8`, defects 19–21 | `ST.8` | explorer and canvas consistency. `C-a` is answerable by now, and `C.8` lands beside `I.8`. **`C.2`, `C.4`, `C.5`, `C.7` and `C.10` were taken out of turn on 2026-08-20** |
...
> **The next batch, in order** (Clay, 2026-08-21). Read this before picking anything.
>
> **The working rule changed today: finish the block migration in full, then work the bugs.**
> Fixing polish between rows has not been paying — three sittings of repair work moved `ST.4`
> by two rows. `ST.8` is parked (see *Stories*), and stream `C` parks with it.
>
> | | Rows | Why here |
> |---|---|---|
> | **1** | `B.5`'s remainder | nothing in `src/` derives containment, and the property test the row required does not test the property. `B.6a` is written against `B.5` |
> | **2** | `B.6a` → `B.6b` → `B.6c` | **in this order and no other.** Definitions, then readers, then the field. `B.6c` must not ship without `B.17` |
> | **3** | `B.8` → `B.9`, then `B.11` → `B.16`, `B.26`, `B.27` | the rest of wave 1 and wave 2 |
> | **4** | `B.17` | **the migration, last.** Nothing in stream `B` ships without it |
> | **then** | the bug list | tasks.md **17–33**, and `ST.8` unparked once the model has stopped moving |
>
> **The exception that was allowed through**: a relationship meeting an interface did not draw
> at all — fixed 2026-08-21, because every later `B` row has a browser clause and none of them
> could be driven honestly on a canvas that silently dropped lines.
...
> at `B.5`.** Every `C` repair has since landed.
...
| **Untouched** | `Y.10`, `T.3`, `S7` | `T.3` has its harness (`T.5` ✓); `S7` stays last so it splits a finished `Files.tsx`. Wave 0's guards (`T.6` ✓, `T.7` ✓) landed 2026-08-20 and pass |
...
### Bugs found on the way — 36, two fixed, two closed by rows
...
| **O.2** | **A resource comes in the same way.** `B.20` makes a resource block hold a workspace-relative path; this is where a path may point at a repo file. **No bytes in the log** — the path travels, the content does not | B.20, O.1 |
| **O.3** | **The translator seam, proven by the site.** A translator reads a project and emits an **artifact**; it never writes back, and the action that runs one may record a `resource` block pointing at the output. **The site is the first target** — a static, readable rendering of a project. **The seam is the deliverable, the site is the proof** | O.1 |
...
**`T.6` and `T.7` landed 2026-08-20 — [landed.md](landed.md).** They were a different job from
`T.3`: that is *cover for three files*, these were **guards against a class of defect the suite
could not express** — a canvas that does not draw, and an arrow pointed the wrong way. **The rule
they set stays**: a T row that passes the moment it is written is not guarding anything. Both were
red when written; `C.11` and `C.9` turned them green.

| | Does | Waits |
|---|---|---|
| **T.3 ⚠** | **`App.tsx`, `Files.tsx` and `Panel.tsx` get cover** — ~2,200 lines, no tests, and the place every browser-found bug lived. **The design has started moving again**: `B.3` deletes `page/kind.ts` and rewrites App and Files, `B.4` rewrites the explorer roles, `B.9` moves display state into App. *Do not write tests for a design that is still moving* applies exactly as it did through U and V — **this now waits on `B.9`**, not on `T.5` alone | T.5, B.9 |
...
| | Does | Waits |
|---|---|---|
| **S7 ⚠** | **Run it after `B.4`, not before.** `B.3` and `B.4` both rewrite `Files.tsx` — the kind derivation out, the roles redrawn — so cutting the seam first means cutting it twice. **Cut the `page/Files.tsx` seam.** Take the split from what the file actually does rather than from line count: the tree and its rows, the naming prompt, the offer menu, and the foot (undo / redo / last action) are four jobs in one file. **Not a rewrite** — the same behaviour, moved, with the browser drive proving each half still works. Fold in the duplicated `ORDER` while here if it falls out naturally; do not go looking for it | ⊘ |
