...
**Startable now**: `Y.1` (the rail seam), `Y.7` (drop `color`, add the dials — the ramp it waited on
exists), and `Y.6`'s remainder. `Y.2`–`Y.4` wait on `Y.1`; `Y.9` waits on `Y.7`.
