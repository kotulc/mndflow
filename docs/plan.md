...
| **B.5 ◐** | **landed — 2026-08-20.** `isReference`, `isContained` and `isPart` now derive the two link roles from the stored `of` link and root references; property tests cover parts, references and deletion-safe filed roots. **Short:** `actions/elements.ts` mutation payload shape was left unchanged |
