...
**Wave V continues** and carries no `◆`. Everything that waited on the icon set has landed —
**V.1, V.2, V.3, V.4, V.5, V.9+V.10, V.11 and V.15 are in [landed.md](landed.md)**. What is left of
the wave is `V.6` (one export door), `V.7` (flow to the bottom right), `V.12`–`V.14` (delete a
project, confirm it, the `＋` rule), `V.16` (a tie can be selected) and `V.17` (the two top groups).
