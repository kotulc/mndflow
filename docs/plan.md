...
| **ST.8** | **Interaction is consistent, and the app always answers back.** Every part you can adjust by hand — a card, a frame edge, a block anchor, an interface — targets the same way, lights the same way, and moves smoothly. Nothing is aim-and-hope, and no two surfaces teach different gestures for the same act | `C.1` → `C.11` | **started out of turn, 2026-08-20.** `C.4` ✓ `C.5` ✓ `C.2` ✓ `C.11` ✓; `C.7` ✓. Remaining repair is `C.10`. Consistency is still the goal; the individual features are how it is reached |
...
| **5** | `C.1`, `C.3`, `C.6`, `C.8`, defects 19–21 | `ST.8` | explorer and canvas consistency. `C-a` is answerable by now, and `C.8` lands beside `I.8`. **`C.2`, `C.4`, `C.5` and `C.7` were taken out of turn on 2026-08-20** and remaining repair `C.10` is in *The next batch* above, not here |
...
> | | Rows | Why here |
> |---|---|---|
> | **1** | `C.10` | **the last repair owed on the 2026-08-20 batch.** `C.9` landed — the resolver is `src/modules/named.ts`, and `T.7` holds the map — so the one surviving card can take it. Closes `ST.8`'s repair list |
> | **2** | `B.3` → `B.9` | **wave 1 resumes where it was left.** Nothing above touches the model core. **Fix `B.3`'s `owns` first** — its *done when* edits `modules/view/index.ts`, which its owns does not list, and an `implement-row` agent will block on that |
>
> **`N.1`'s remainder and `N.8` stay in wave 3** — they are not repairs owed on this batch; they are
> the rest of a row and a hole `N.5` closes on its way past.
>
> **Wave 0 and the other three repairs are done** — `T.6`, `T.7`, `C.9`, `C.11`, and with them
> `C.2` and `C.7` complete. All six are in [landed.md](landed.md).
...
> at `B.3`.** The remaining `C` repair (`C.10`) is the exception — they are owed on work already in
> the tree, so they come before anything new in `C`. `C.11` landed.
