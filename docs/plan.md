...
| | The goal | Rows | State |
|---|---|---|---|
| **ST.1** | **Reorganising is easy.** Move a block anywhere it could sensibly go — into another branch of the same project, into a different project, or out to the workspace — and see where it is going to land while dragging. Nothing announces itself afterwards, because the move was obvious as it happened | `P.1` ✓ ([landed](landed.md)), `P.13`, `P.14`, `P.15` | rows landing |
...
**Wave Y is down to three rows** — `Y.6a`, `Y.9`'s sibling `Y.10`, and nothing else. The style
surface closed with four dials (`slot`, `emphasis`, `weight`, `voice`): no colour, no pixel count,
no font. `Y.1`–`Y.9` are in [landed.md](landed.md).

**One `◐` row is still work** — `U.18` left `tray.full`, and handed it to `W.1`, which owns the size
and the door that reaches it together. `Y.4` and `Y.6` both closed theirs and are archived.
