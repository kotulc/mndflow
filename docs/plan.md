...
|---|---|---|
| **1** | **The kit at HEAD.** `release/kit.json` names 0.3.0 from before holders, packages, `source` and marks, and `packages/kit/.types` still carries retired modules. Re-pack, stamp, and clear the stale types | nothing outside the repo can build against the current schema until it lands |
