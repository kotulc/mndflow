...
| **31 ◐** | **`isPart` has no caller in `src/`.** `isContained` got one on 2026-08-21 — `cascades()` gates the live `delete_element` sweep and `drop.run`'s payload. `isPart` cannot get an honest one at that site: `isContained` implies `isReference` implies `!isPart`, so any call there is provably dead, and two attempts were caught in review. **Open question, not a wiring gap** — either `B.5`'s done-when (a) asked for something untrue of this call site, or `isPart`'s real consumer is a row nobody has written | `graph/fold.ts` |
| ~~**32**~~ **closed 2026-08-21** — the test now folds the graph and asserts the contained reference survives while an ordinary child is swept; a second case in `tests/actions/elements.test.ts` asserts the `delete` payload never targets the survivor. *Original:* **`B.5`'s property test does not test its property.** *Deleting a container never deletes a contained root* is asserted by checking `isContained` on an **un-folded local object**, plus `ROOT` surviving (undeletable by construction) and the contained reference **being** deleted with its container. It passes whether or not the property holds. Carried by `B.5 ◐` | `tests/graph/fold.test.ts` |
