                ...
                <table className="contents-table">
                  ...
                  <tbody onMouseLeave={() => onHover?.(null)}>
                    {shown.map((row) => {
                      ...
                      return (
                        <tr key={row.id} className={picked.includes(row.id) ? "picked" : ""}
                            onMouseEnter={() => onHover?.(row.id)}
                            onClick={() => pick_row(row.id)}>
                          {HEAD.map((h) => (
                          ...
                          <td className="view">
                            {/* **Only on the row picked, and only when it is
                                elsewhere** — one that is here already lights. */}
                            {onView && picked.includes(row.id) && home !== layer ? (
                    ...
                    {shown.length === 0 ? (
                      <tr className="empty">
                        <td colSpan={HEAD.length + columns.length + 1}>
