...
import { ALIGNS, DECORS, EMPHASES, FILLS, HUE, INTENSITY, PLACES, SHEERS,
         SLOTS, STEPS, VOICES, WEIGHTS, type Act, type Id } from "@mnd/core";
import { names, type IconName } from "@mnd/theme";

/** Which question a tab asks, in the one word for it. */
const TABS = ["color", "contrast", "fill", "border", "text", "marked",
              "icon", "name", "label", "aligned"] as const;
export type Part = (typeof TABS)[number];

/** One question, and where its answer lives. **The tab is its name**, so it
 *  carries no word of its own — a heading repeating the tab above it was one
 *  label too many. */
type Question = { name: string; key: "card" | "style"; tip: string;
...
const ALSO: Partial<Record<Part, Question>> = {
  fill: { name: "sheer", key: "style",
    tip: "How far the fill lets the ground through. The fill alone — never the "
       + "ink and never the line, and never all the way to nothing.",
    of: plain(SHEERS) },
  border: { name: "line", key: "style",
    tip: "Which rung of the ramp the border takes. Overrules contrast.",
    of: plain(STEPS) },
  text: { name: "ink", key: "style",
    tip: "Which rung of the ramp the writing takes. Overrules contrast.",
    of: plain(STEPS) },
};
...
function Options({ q, said, chain, set, off }: {
  ...
  return (
    <div className="options" title={q.tip} aria-disabled={off || undefined}>
