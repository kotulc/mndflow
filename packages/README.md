...
```
project(graph, layer, config) → Scene { nodes, edges, perches, slots, frame, trail }
```
...
**Where a line runs is the projection's; what answers a click is the library's.** `views/route.ts`
routes a run round the cards it passes, so the canvas and the SVG export draw the same path. Hit
testing, the viewport and the drag stay React Flow's — a boundary that exists to re-implement those
is not a boundary worth keeping.
...
| views | no overlap, containment, stability under reorder, seats, and Scene invariants over text and SVG projections of **shape, not coordinates** | no |
