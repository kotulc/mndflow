...
type Props = {
  ...
  className?: string;
  placeholder?: string;
  /** Whether this name is already taken by a sibling. Omitted where nothing
   *  competes for the name — a relation kind, say — and then nothing is ever
   *  refused. */
  ...
  /** Keep pointer and key events off the canvas underneath. React Flow listens
   *  natively on the node, so a React handler stopping them comes too late —
   *  the class is what actually holds the drag and pan off. */
  guarded?: boolean;
};

export function NameField({ initial, className, placeholder, taken, onCommit,
                           onCancel, onSay, guarded }: Props) {
  const [clash, setClash] = useState(false);
  const spoken = (name: string) => Boolean(taken?.(name));
  ...
  return (
    <>
      <input
        className={`${className ?? ""}${guarded ? " nodrag nopan" : ""}${clash ? " clash" : ""}`}
        autoFocus
        defaultValue={initial}
        placeholder={placeholder}
        onPointerDown={guarded ? (event) => event.stopPropagation() : undefined}
        onClick={guarded ? (event) => event.stopPropagation() : undefined}
        onDoubleClick={guarded ? (event) => event.stopPropagation() : undefined}
        // Warned while typing, so the refusal is never a surprise at the end.
        onChange={(event) => setClash(spoken(event.target.value))}
        onBlur={(event) => (spoken(event.target.value) ? onCancel() : onCommit(event.target.value))}
        onKeyDown={(event) => {
          if (guarded) event.stopPropagation();
