/** One card on the diagram.
 *
 *  How it is composed — layout, shape, label, which fields show — comes from
 *  the card component (`cardOf` / `PLAIN`). Colour comes from `lookOf`. The
 *  engine still places a rectangle; `outline` is what is stroked inside it.
 *
...
/** SVG overlay for a non-rect outline. Inline rather than a stylesheet rule —
 *  the visual style sheet is frozen while the graph model settles. */
const SHAPE: CSSProperties = {
  position: "absolute", inset: 0, width: "100%", height: "100%",
  pointerEvents: "none", overflow: "visible",
};

/** Stroke the card module's outline inside the engine's box. Seats and routes
 *  still meet the rectangle; this is only what is drawn. */
...
export const NodeCard = memo(({ data, selected, positionAbsoluteX = 0,
  ...
  // The claim the component surface is measured against: today's card is one
  // configuration among others, read here rather than hard-wired below.
  ...
  const title = titled && (
    <div className={`card-head${
      ...
      {/* A subtype where one was set, otherwise what this module calls a
          ...
          name has to stay put when a child is added. `layout: type` is PLAIN. */}
      {chipped && (
