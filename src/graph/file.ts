...
import {
  ROOT, edge as newEdge, element as newElement, field as newField, type Definition, type Edge,
  type Element, type Field, type Graph,
} from "./types";
...
export function write(graph: Graph, id: string, steps: number): string {
...
export function read(raw: unknown): Envelope | null {
  ...
  const held = it.graph as Record<string, unknown>;
