...
import {
  actual, blocksOf, edgesIn, fieldsOf, groupsIn, isContainer, isProxy, membersOf, nameOf, notesIn,
  typeName,
  portsOf, tiesOf,
} from "../graph/fold";
import type { Dir, Field, Flow, Graph } from "../graph/types";
...
export function Contents(props: Props) {
  ...
  const rows = useMemo<Row[]>(() => {
    ...
    for (const { attr } of groupsIn(graph, view)) {
      ...
      out.push({
        ...
        type: typeName(graph, attr.type), edge: false, renameable: true, body: said(attr.id),
      });
    }
          ...
          <div className="carries">
            ...
            {held.map((held) => (
              <span className="held value" key={held.name}>
                ...
                <Draft
                  value={held.value}
                  placeholder={held.unit ? `value (${held.unit})` : "value"}
                  onCommit={(next) => onUpdateField(row.id, held.name, { value: next })}
                />
  ...
  return (
    <div className="contents" onMouseLeave={() => (onHint(null), setOver(null))}>
      ...
      {over && (over.body || fieldsOf(graph, over.id).length > 0) && (
