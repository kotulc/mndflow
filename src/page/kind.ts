 ...
 *  all, reads structure. A reference reads as whatever it points at, one hop,
 ...
 *  references all pointing at structure blocks read as a structure layer.
 *
...
import { blocksOf, isReference } from "../graph/fold";
...
/** What a child stands for: itself, or one hop through a reference. Unresolved —
...
function homeOf(
  ...
  if (!isReference(node)) return { graph, element: node };
 ...
 *  every project currently held, for a reference that points across one — absent
